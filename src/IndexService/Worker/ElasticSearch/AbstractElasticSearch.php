...
use OpenDxp\Bundle\EcommerceFrameworkBundle\IndexService\Config\ElasticSearchConfigInterface;
...
use OpenDxp\Bundle\EcommerceFrameworkBundle\IndexService\Worker;
...
abstract class AbstractElasticSearch extends Worker\ProductCentricBatchProcessingWorker implements Worker\BatchProcessingWorkerInterface
{
    ...
    protected function doDeleteFromIndex(int $objectId, ?IndexableInterface $object = null): void
    {
        ...
        $storeEntry = \OpenDxp\Db::get()->fetchAssociative('SELECT * FROM ' . $this->getStoreTableName() . ' WHERE  id=? AND tenant=? ', [$objectId, $this->getTenantConfig()->getTenantName()]);
        if ($storeEntry) {
            $isLocked = $this->checkIndexLock(false);
            if ($isLocked) {
