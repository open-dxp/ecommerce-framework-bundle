...
use OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\Order\Listing\Filter\OrderDateTime;
use OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\Order\Listing\Filter\OrderSearch;
use OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\Order\Listing\Filter\ProductType;
...
class AdminOrderController extends UserAwareController implements KernelControllerEventInterface
{
    ...
    #[Route('/list', name: 'opendxp_ecommerce_backend_admin-order_list', methods: ['GET'])]
    public function listAction(Request $request, IntlFormatter $formatter, PaginatorInterface $paginator): Response
    {
        ...
        return $this->render('@OpenDxpEcommerceFramework/admin_order/list.html.twig', [
            'paginator' => $paginator,
            'opendxpUser' => \OpenDxp\Tool\Admin::getCurrentUser(),
            'listPricingRule' => new \OpenDxp\Bundle\EcommerceFrameworkBundle\PricingManager\Rule\Listing(),
            'defaultCurrency' => Factory::getInstance()->getEnvironment()->getDefaultCurrency(),
            'formatter' => $formatter,
        ]);
    }

    #[Route('/detail', name: 'opendxp_ecommerce_backend_admin-order_detail', methods: ['GET'])]
    public function detailAction(
        ...
        $opendxpSymfonyConfig = $this->getParameter('opendxp.config');
         ...
         * @return string
         */
        $geoPoint = function (array $address) use ($opendxpSymfonyConfig, $client) {
            $baseUrl = $opendxpSymfonyConfig['maps']['geocoding_url_template'];
            $url = str_replace(
                '{q}',
                urlencode(
                    $address[0]
        ...
        // get geo points
        $invoiceAddressCacheKey = 'opendxp_order_invoice_address_' . $order->getId();
        ...
        if ($order->getDeliveryStreet() && $order->getDeliveryZip()) {
            $deliveryAddressCacheKey = 'opendxp_order_delivery_address_' . $order->getId();
        ...
        return $this->render('@OpenDxpEcommerceFramework/admin_order/detail.html.twig', [
            'opendxpUser' => \OpenDxp\Tool\Admin::getCurrentUser(),
            ...
            'opendxpSymfonyConfig' => $opendxpSymfonyConfig,
            'formatter' => $formatter,
    ...
    #[Route('/item-cancel', name: 'opendxp_ecommerce_backend_admin-order_item-cancel', methods: ['GET', 'POST'])]
    public function itemCancelAction(Request $request, CsrfProtectionHandler $csrfProtection): Response
    {
        ...
        if ($request->get('confirmed') && $orderItem->isCancelAble()) {
            ...
            // redir
            $url = $this->generateUrl('opendxp_ecommerce_backend_admin-order_detail', ['id' => $order->getId()]);

            return $this->redirect($url);
        }

        return $this->render('@OpenDxpEcommerceFramework/admin_order/item_cancel.html.twig', [
            'orderItem' => $orderItem,
        ]);
    }

    #[Route('/item-edit', name: 'opendxp_ecommerce_backend_admin-order_item-edit', methods: ['GET', 'POST'])]
    public function itemEditAction(Request $request, CsrfProtectionHandler $csrfProtectionHandler): Response
    {
        ...
        return $this->render('@OpenDxpEcommerceFramework/admin_order/item_edit.html.twig', [
            'orderItem' => $orderItem,
        ]);
    }

    #[Route('/item-complaint', name: 'opendxp_ecommerce_backend_admin-order_item-complaint', methods: ['GET', 'POST'])]
    public function itemComplaintAction(Request $request, CsrfProtectionHandler $csrfProtectionHandler): Response
    {
        ...
        return $this->render('@OpenDxpEcommerceFramework/admin_order/item_complaint.html.twig', [
            'orderItem' => $orderItem,
        ]);
    }
}
