...
opendxp.registerNS("opendxp.bundle.EcommerceFramework.pricing.config.panel");

/**
 * @private
 */
opendxp.bundle.EcommerceFramework.pricing.config.panel = Class.create({
    ...
    initialize: function(layoutId) {
        ...
        var _this = this;
        Ext.Ajax.request({
            url: Routing.generate('opendxp_ecommerceframework_pricing_get-config'),
            method: "GET",
        ...
        if (!this.tree) {
            this.saveButton = new Ext.Button({
                ...
                handler: function() {
                    ...
                    // save order
                    Ext.Ajax.request({
                        url: Routing.generate('opendxp_ecommerceframework_pricing_save-order'),
                        params: {
            ...
            var store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_ecommerceframework_pricing_list'),
                    reader: {
                        type: 'json'
                    }
                }
    ...
    addRuleComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 2 && regresult == value) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_ecommerceframework_pricing_add'),
                method: 'POST',
                params: {
                    name: value,
    ...
    deleteRule: function (tree, record) {
        ...
        opendxp.helpers.deleteConfirm(
            ...
            function () {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_ecommerceframework_pricing_delete'),
                    method: 'DELETE',
                    params: {
                        id: record.id
                    },
    ...
    copyRule: function (tree, record) {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_ecommerceframework_pricing_copy'),
            method: 'POST',
            params: {
                id: record.id
            },
    ...
    renameRuleComplete: function (options, button, value, object) {
        ...
        if (button == 'ok') {
            ...
            if(value.match(/^[a-zA-Z0-9_\-]+$/)) {
                ...
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_ecommerceframework_pricing_rename'),
                    method: 'PUT',
                    params: {
                        id: options.id,
                        name: value
    ...
    openRule: function (tree, record, item, index, e, eOpts ) {
        ...
        //try {
            ...
            if (this.panels[pricingRuleKey]) {
                ...
                // load defined rules
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_ecommerceframework_pricing_get'),
                    params: {
                        id: record
                    },
                    success: function (response) {
                        var res = Ext.decode(response.responseText);
                        var item = new opendxp.bundle.EcommerceFramework.pricing.config.item(this, res);
