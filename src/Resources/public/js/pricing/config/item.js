...
opendxp.registerNS("opendxp.bundle.EcommerceFramework.pricing.config.item");
/**
 * @private
 */
opendxp.bundle.EcommerceFramework.pricing.config.item = Class.create({
    ...
    initialize: function(parent, data) {
        ...
        this.tabPanel = new Ext.TabPanel({
            ...
            forceLayout: true,
            id: "opendxp_pricing_panel_" + this.data.id,
            buttons: [{
                text: t("save"),
    ...
    getSettings: function () {
        ...
        var langTabs = [];
        Ext.each(opendxp.settings.websiteLanguages, function(lang){
            var tab = {
                title: opendxp.available_languages[ lang ],
                items: [{
                    xtype: "textfield",
                    name: "label." + lang,
                    fieldLabel: t("label"),
    ...
    getConditions: function() {
        ...
        var _this = this;
        var addMenu = [];
        var itemTypes = Object.keys(opendxp.bundle.EcommerceFramework.pricing.conditions);
        ...
        Ext.each(this.parent.condition, function (condition) {
            ...
            if(itemTypes.indexOf(method) != -1)
            {
                addMenu.push({
                    ...
                    text: opendxp.bundle.EcommerceFramework.pricing.conditions[method](null, null,true),
    ...
    getActions: function () {
        ...
        var _this = this;
        var addMenu = [];
        var itemTypes = Object.keys(opendxp.bundle.EcommerceFramework.pricing.actions);
        ...
        Ext.each(this.parent.action, function (action) {
            ...
            if(itemTypes.indexOf(method) != -1)
            {
                addMenu.push({
                    iconCls: "bundle_ecommerce_pricing_icon_" + method,
                    text: opendxp.bundle.EcommerceFramework.pricing.actions[method](null, null,true),
    ...
    addCondition: function (type, data) {
        ...
        // create condition
        var item = opendxp.bundle.EcommerceFramework.pricing.conditions[type](this, data);
        ...
        item.on("afterrender", function (el) {
            ...
            var leftBracket = el.getEl().insertHtml("beforeEnd",
                '<div class="opendxp_targeting_bracket opendxp_targeting_bracket_left">(</div>', true);
            var rightBracket = el.getEl().insertHtml("beforeEnd",
                '<div class="opendxp_targeting_bracket opendxp_targeting_bracket_right">)</div>', true);

            if(data["bracketLeft"]){
                leftBracket.addCls("opendxp_targeting_bracket_active");
            }
            if(data["bracketRight"]){
                rightBracket.addCls("opendxp_targeting_bracket_active");
            }

            // open
            leftBracket.on("click", function (ev, el) {
                var bracket = Ext.get(el);
                bracket.toggleCls("opendxp_targeting_bracket_active");
    ...
    addAction: function (type, data) {
        ...
        var item = opendxp.bundle.EcommerceFramework.pricing.actions[type](this, data);
    ...
    save: function () {
        ...
        for (var i=0; i<conditions.length; i++) {
            ...
            // get the brackets
            condition["bracketLeft"] = Ext.get(conditions[i].getEl().query(".opendxp_targeting_bracket_left")[0])
                                                                .hasCls("opendxp_targeting_bracket_active");
            condition["bracketRight"] = Ext.get(conditions[i].getEl().query(".opendxp_targeting_bracket_right")[0])
                                                                .hasCls("opendxp_targeting_bracket_active");
        ...
        // send data
        Ext.Ajax.request({
            url: Routing.generate('opendxp_ecommerceframework_pricing_save'),
            params: {
                id: this.data.id,
    ...
    saveOnComplete: function (response) {
        ...
        if (response.success) {
            ...
            opendxp.helpers.showNotification(t("error"), t(response.message), "error", );
        }
    },
    ...
    recalculateBracketIdent: function(list) {
        ...
        list.each(function (condition) {
            ...
            if(item.select('.opendxp_targeting_bracket_left.opendxp_targeting_bracket_active').getCount() == 1)
            {
                ident++;
            }
            // check if a bracket is close
            else if(item.select('.opendxp_targeting_bracket_right.opendxp_targeting_bracket_active').getCount() == 1)
            {
                if(ident > 0) {
                    ident--;
                }
 ...
 * CONDITION TYPES
 */
opendxp.registerNS("opendxp.bundle.EcommerceFramework.pricing.conditions");
opendxp.bundle.EcommerceFramework.pricing.conditions = {
    ...
    getTopBar: function (name, index, parent, data, iconCls) {
        ...
        return [{
            ...
            handler: function (blockId, parent) {
                ...
                var index = opendxp.bundle.EcommerceFramework.pricing.conditions.detectBlockIndex(blockElement, container);
    ...
    conditionCatalogProduct: function (panel, data, getName) {
        ...
        var item =  new Ext.form.FormPanel({
            ...
            items: [
                new opendxp.bundle.EcommerceFramework.pricing.config.objects(data.products, {
                    name: "products",
                    title: "",
                    visibleFields: "path",
                    height: 200,
    ...
    conditionCatalogCategory: function (panel, data, getName) {
        ...
        var item =  new Ext.form.FormPanel({
            ...
            items: [
                new opendxp.bundle.EcommerceFramework.pricing.config.objects(data.categories, {
                    name: "categories",
                    title: "",
                    visibleFields: "path",
                    height: 200,
    ...
    conditionVoucherToken: function (panel, data, getName) {
        ...
        var item =  new Ext.form.FormPanel({
            ...
            items: [
                new opendxp.bundle.EcommerceFramework.pricing.config.objects(data.allowList, {
                    classes: [
 ...
 * ACTION TYPES
 */
opendxp.registerNS("opendxp.bundle.EcommerceFramework.pricing.actions");
opendxp.bundle.EcommerceFramework.pricing.actions = {
