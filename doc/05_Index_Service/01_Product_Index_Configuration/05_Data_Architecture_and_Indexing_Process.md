...
For indexing itself the provided OpenDxp console commands can be used. 


## Simple Mysql Architecture
- OpenDxp object data is transferred directly to the Product Index. 
- After every update of a OpenDxp object, the changes are directly written into the Product Index. 
- Only used for `\OpenDxp\Bundle\EcommerceFrameworkBundle\IndexService\Config\DefaultMysql`
...
- By default, OpenDxp assumes that the product class is `OpenDxp\Model\DataObject\Product`. If you have a 
...
- In this case a so called store table is between the OpenDxp objects and the *Product Index*. This store table enables to ...
   ...
   - ... update the *Product Index* asynchronously and therefore update also dependent elements (children, variants, ...) 
         of an updated OpenDxp object without impact on save performance. 
   - ... rebuilding the whole *Product Index* out of the store table much faster since no direct interaction with 
         OpenDxp objects is needed. 

- After every update of a OpenDxp object, the changes are written into the store table and all child objects of the 
...
- For process the preparation queue and update OpenDxp objects to the index store table, use following command. 
...
- For manually update all OpenDxp objects in the index store use following command. As stated before, this should only be
  necessary for an initial fill-up of the index. After that, at least Product Index Store and OpenDxp objects should always 
