...
which needs to implement `\OpenDxp\Bundle\EcommerceFrameworkBundle\CheckoutManager\CommitOrderProcessorInterface`.
...
The default implementation `\OpenDxp\Bundle\EcommerceFrameworkBundle\CheckoutManager\V7\CommitOrderProcessor` provides 
basic functionality like creating a OpenDxp order object and sending an order confirmation mail.

Order creation itself is delegated to the `\OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\OrderManagerInterface`.
...
* ...to extend `\OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\V7\OrderManager` and overwrite the method `applyCustomCheckoutDataToOrder` 
  to add additional custom fields to the order object and 
* ...to extend `\OpenDxp\Bundle\EcommerceFrameworkBundle\CheckoutManager\V7\CommitOrderProcessor` and overwrite the method 
...
class OrderManager extends \OpenDxp\Bundle\EcommerceFrameworkBundle\OrderManager\V7\OrderManager
{
    /**
